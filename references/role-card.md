...
4. **Create documentation.** Write dated design/feature/change/fix records with actual test evidence and timeline events. Create deliverables (`changelog.md`, `functional-description.md`, `api-spec.md`) at the role directory root. Before adding, removing, or reorganizing owned application files outside the accepted scope, report the requested boundary change to Scheduler for Governance handling. Resume only after the accepted scope and assignment are updated; do not self-expand authority. Capability roles maintain their own implementation reports in their accepted documentation scope; they do not grant themselves more authority.
...
Bootstrap creates only the role definition: `role-card.md` and `boundary.md`. All other documents (`changelog.md`, `functional-description.md`, `api-spec.md`) are created by the role during implementation.
