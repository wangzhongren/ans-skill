...
The [dashboard server](../dashboard/server.py) serves the [HTML viewer](../dashboard/index.html) using Python 3 standard-library HTTP tools. The old `scripts/serve_dashboard.py` path remains a compatibility entry point. It reads the [coordination records](coordination.md); it neither dispatches workers nor authorizes actions. Python is the tool implementation language, not a business-project requirement. For a shared server, follow the [standalone Dashboard deployment guide](../dashboard/README.md), including its Docker Compose and optional `--base-path` settings: a local collector synchronizes only the project display projection, not the source tree. Initialize the project-root `.ans-dashboard.local.json` with `python3 -m dashboard.local_config init`; it contains the Key and must remain untracked and out of prompts. When a prefix is configured, include it in the sync client's `--server-url`.

Role preflight uses `python3 scripts/check_dashboard_sync.py --root <project-root>` from the installed Skill. It reports whether the project has cloud configuration, whether a continuous sync process holds the local lease, and the last successful upload time. No Key is read or displayed. For an unconfigured project, ask once whether the customer wants shared cloud synchronization; for a stopped configured project, report stale cloud state and resume only within existing authorization. The local Dashboard itself remains read-only and does not launch the sync process.
...
The dedicated **项目理解** view shows role purpose and mutation paths from its role card and boundary document, plus four direct overview tabs: Flow, Data, Interface, and Definition. Each tab shows its category summary. Data overviews show concrete fields. The Interface overview separates **网络接口** from **内部接口**: network entries show protocol, method when HTTP, request URL and fields; internal entries show their public entry and fields. Clicking an item opens its detail. A Flow detail shows direct trigger conditions—including button activation when applicable—ordered steps, input and output data, and related interfaces. Related data/interface details appear inside the Flow detail without an extra click. Clicking a role card opens this view for that role. The index refreshes from the [shared SQLite store](project-context.md) with the dashboard snapshot; item details are read only when selected or referenced by a flow. Missing context is shown as missing, not synthesized from another role or from the sample project. The separate **协作事件** view contains task/design history and does not track live UI activations.
...
Local mode is an observation tool, not an authenticated multi-user deployment. Do not expose that mode through a public proxy. The separate cloud mode has built-in login: every active user can view all projects, while administrators alone manage accounts, Keys and decisions. Project Keys remain scoped to one project and receive bounded projections from local collectors. See [deployment and data boundary](../dashboard/README.md). Local records larger than 2 MiB are reported instead of loaded; the viewer is intended for bounded task records, not unbounded production logs.

In shared cloud mode only, the optional [role channel](../dashboard/README.md#role-channel) adds messages, exact permission requests, one administrator decision per request, and per-project audit events. Each role sends using the project's shared Key and declares its own role ID; the server authenticates the project rather than the individual role. All active users can view the channel, while only an administrator may record a decision. This is communication, not agent execution. Even an approved request does not satisfy `task_ops` consent/configuration checks or activate a role by itself.
