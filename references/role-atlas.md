...
Open the Dashboard at the printed localhost URL and select its **角色功能图谱** tab. Role details also have a direct entry that selects that role. The walkthrough is embedded within the same Dashboard; leaving the tab pauses playback and returning preserves the current step. `/role-atlas` remains a standalone view for direct inspection, not the primary navigation path. Use `--role ROLE_ID` to rebuild one role while preserving other index entries; `--roles relative/path` selects a nonstandard role directory. Default output is the project-root `doc/role-atlas/`, with index.json and one hashed filename per role. An explicit `--out` is available for exporting data elsewhere; the bundled server currently reads the default location.
