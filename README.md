...
这套技能帮助 AI 在项目里**只改获准的文件、把代码放到合适的位置、用测试说明改动有效**。每个项目默认可开一个本地 Dashboard，看角色、流程和真实进度。需要多人查看时，再接云端 Dashboard；它只接收展示所需的信息，不上传整个仓库。

通用页面、服务和规则都在技能目录中。业务项目只提供自己的角色卡、设计文档和协同记录；`test/game-engine` 是验证样例，不是运行其他项目的依赖。

第一次接触这套技能，先看[白话说明](SKILL.zh.md)；要看目录、数据流和命令，再看[完整总览](doc/guide/2026-09-25_guide_ans-skill-overview.md)。

## 能做什么

1. **知道谁能改什么**：每个角色有自己的文件清单；清单外的文件不能顺手修改。
2. **知道代码怎么配合**：界面接收请求，流程安排步骤，业务层处理规则，存储层访问外部资源；只测试这次实际改动的部分。
3. **看到真实进度**：Dashboard 读取角色和任务记录，显示谁在做、哪里卡住、哪些工作已验证；没有记录就不编造。
4. **看懂具体流程**：先说明流程帮用户做什么、成功或失败会怎样，再看 `if/else` 分支、字段、接口和排查位置。
5. **按需接云端**：多人查看或交流时才同步展示信息；云端的批准不会自动允许本地 AI 写代码。
...
项目理解保存在一份 `project-context/context.sqlite3` 中，以 `role_id` 区分角色。Dashboard 提供流程、数据、接口和定义总览。**每条流程先说明它帮用户做什么、成功后会怎样、失败时会怎样**；详情再画实际的 `if/else` 分支，点异常节点可看代码位置和排查方法。旧流程只有步骤时只画顺序，不猜分支。数据和接口总览展示具体字段，网络接口还展示请求方法与 URL。角色职责与边界来自角色卡和边界文档。AI 用 [项目理解工具与写法](references/project-context.md) 按角色查询或更新。

原有第 6 版项目理解数据库仍可查看。要给旧项目写入新的用途说明或分支图，先备份 `project-context/context.sqlite3`，确认项目已授权升级共享数据库，再运行 `scripts/context_store.py ... migrate` 升级到第 7 版；具体命令见[项目理解说明](references/project-context.md)。
