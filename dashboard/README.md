...
云端快照包括角色名称、职责摘要、边界路径、项目理解的流程/数据/接口/定义条目，以及任务状态与协作事件。条目中的说明和字段会被同步，因为页面要展示这些内容；请在本地项目理解里只记录适合授权用户查看的信息。角色文档正文、完整仓库文件与本地绝对路径不会由同步器上传。Dashboard 是观察页，不负责派发角色、修改业务代码或自动验收。
