...
async function refresh(){
  const [projects,users,keys,roles]=await Promise.all([api('/api/admin/projects'),api('/api/admin/users'),api('/api/admin/keys'),api('/api/admin/role-tokens')]);
  ...
  $('roleTokens').replaceChildren(...roles.roleTokens.map(token=>{const item=row('#'+token.id,token.projectId,token.roleId,token.label,token.revokedAt?'已撤销':'有效');if(!token.revokedAt){const button=document.createElement('button');button.type='button';button.className='secondary';button.textContent='撤销';button.onclick=async()=>{try{await post('/api/admin/role-tokens/revoke',{id:token.id});await refresh();message('已撤销角色凭证 #'+token.id)}catch(error){message(error.message)}};item.append(button)}return item}));
}
...
async function init(){
  try{
    ...
    attach('roleTokenForm','/api/admin/role-tokens',()=>({projectId:$('roleTokenProject').value,roleId:$('roleTokenRole').value,label:$('roleTokenLabel').value}),result=>{$('issuedRoleToken').textContent=result.token;$('roleTokenResult').hidden=false;message('角色凭证已创建，仅显示这一次')});
