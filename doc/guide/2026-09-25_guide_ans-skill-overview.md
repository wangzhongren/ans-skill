...
这份文档只讲主线。**ANS Skill 是开发规则和工具的集合，不是业务项目，也不是云端服务。**AI 使用它来确定谁能改哪些文件、代码应放在哪一层、怎样验证；Dashboard 负责展示记录。先在本地把项目做好，需要共享查看时再接云端。
...
flowchart LR
    ...
    Cloud -->|消息和决定可按需查询| Client["本地角色通信 CLI"]
```
...
3. **按五层职责设计和实现。**构建顺序通常是 `Model → Provider → Service → Pipeline → Interface`；根 `main` 文件只负责启动 Interface。运行调用方向是 `Interface → Pipeline → Service → Provider`，每层只调用相邻下一层，Model 可被共享。Provider 和 Service 有 `abstract/` 契约、`impl/` 实现、`public/` 唯一对外入口。`utils/`、`resource/`、`test/`、`docs/` 只是辅助目录，不增加架构层；简单功能无需每层都新增文件。
4. **每层验收后再接上层。**角色写自己的测试和带日期的设计、功能、修改或修复文档；实际成功/失败、文件范围和版本构成证据。多角色协同任务由项目角色管理依赖。需要严格派发门禁时，`task_ops` 使用客户审阅的配置哈希，写入 `plan.json`、`state.json`、`events.jsonl`，再生成 `board.md`；它不自动启动代理。
5. **装配与展示。**装配角色跑根入口和端到端测试。项目角色核对证据后，本地 Dashboard 展示角色、项目理解、阶段任务和协作事件；没有真实调度记录时，任务视图保持空白，不会编造状态。
...
| 历史与设计依据 | 根 `docs/` 和角色自己的 `docs/`，文件名带日期 | 文档详情；可按日期形成时间线 |

项目理解是**当前导航索引**，文档是**设计与变更历史**，任务记录是**执行状态**。三者不互相替代。项目理解中的按钮点击属于流程触发条件；协作事件记录任务与设计变化，不记录每次按钮点击。详见[项目理解格式](../../references/project-context.md)和[任务操作协议](../../references/task-operations.md)。
...
从**技能目录**运行的最短命令如下；把 `<项目目录>` 和服务器地址换成真实值：

```sh
python3 scripts/check_dashboard_sync.py --root <项目目录>
python3 -m dashboard.server --root <项目目录> --port 0

# 只有需要云端时才初始化一次；命令会交互读取 Key
python3 -m dashboard.local_config init \
  --root <项目目录> --project-id orders \
  --server-url https://dashboard.example.com/ans-dashboard

# 持续同步；Ctrl+C 停止
python3 -m dashboard.sync --root <项目目录> --interval 10

# 按需读取该项目的云端消息与申请
python3 -m dashboard.channel_cli --root <项目目录> --role orders list
```
