...
Dashboard 用只读连接读取实际项目数据库，增加独立的“项目理解”视图。点击角色卡进入该角色的流程总览；顶部并列展示流程、事件、数据、接口和定义总览。角色职责与修改边界从角色卡和边界文档读取。数据与接口总览直接展示具体字段，HTTP 接口展示请求方法与 URL；点条目可看完整说明。流程详情直接展示触发事件、执行步骤、输入与输出数据、相关接口和产出事件；事件详情展示触发条件、执行动作与消费方。默认 Dashboard 移除角色功能图谱入口。写入工具要求角色匹配、记录版本匹配，并将删除保留为可恢复标记。它不替代客户授权或 OS 级身份验证。
...
验证：技能结构校验通过；68 项测试通过，覆盖单库角色隔离、分类汇总、事件触发器、流程触发事件和输入输出关系、数据与接口字段、请求 URL、版本 1/2→3 迁移、CRUD 版本冲突、可恢复删除与 Dashboard 只读接口；页面脚本语法检查通过。浏览器在临时项目中核对了角色职责与边界、数据字段表、接口方法与 URL、输入/输出字段，并检查了窄屏排版。默认导航不再出现角色功能图谱；旧生成器仅保留显式使用。没有改动示例业务项目。
