...
events:
  ...
  - date: "2026-09-24"
    kind: changed
    summary: 接口总览区分网络接口和内部接口，显式呈现协议、方法、URL 与字段
  - date: "2026-09-24"
    kind: verified
    summary: 72 项测试、技能校验、页面脚本检查及接口双分组浏览器验证通过
---
...
Dashboard 用只读连接读取实际项目数据库，增加独立的“项目理解”视图。点击角色卡进入该角色的流程总览；顶部并列展示流程、数据、接口和定义总览。角色职责与修改边界从角色卡和边界文档读取。接口总览分组展示网络接口和内部接口，两组都有字段；网络接口另外展示协议、HTTP 方法和请求 URL。点条目可看完整说明。流程详情直接展示触发条件、执行步骤、输入与输出数据和相关接口。按钮点击或键盘激活若启动行为，直接写为流程触发条件；同次 HTTP 请求是流程步骤。SQLite 版本 6 为接口增加种类和协议，旧 HTTP 记录迁移为网络接口。旧事件行及关联保留读取兼容；`validate` 检查活动流程是否缺少触发条件。默认 Dashboard 移除角色功能图谱入口。写入工具要求角色匹配、记录版本匹配，并将删除保留为可恢复标记。它不替代客户授权或 OS 级身份验证。
...
验证：技能结构校验通过；72 项测试通过，覆盖单库角色隔离、流程触发条件、数据与接口字段、网络/内部接口分类、HTTP 请求 URL、版本 1–5→6 迁移、CRUD 版本冲突、可恢复删除与 Dashboard 只读接口；页面脚本语法检查通过。浏览器在临时项目中确认接口总览同时显示“网络接口”“内部接口”两组，网络条目展示 `POST /api/orders/export` 与输入输出字段，内部条目展示 `OrderService.public.export` 与字段。没有改动示例业务项目。
